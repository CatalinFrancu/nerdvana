...
//
// Metodă: La fel cu cata-brute.cpp, dar folosim algoritmul lui Lee în loc de
// flood fill. Acesta este ceva mai rapid și evită problemele de depășire a
// stivei.
#include <stdio.h>
...
void lee(short r, short c, int s1, int s2) {
  int head = 0, tail = 1;
  queue[0] = { r, c };
  s[r][c] = s2;
...
bool conn_test(int r1, int c1, int r2, int c2) {
  ...
  lee(r1, c1, ST_ALIVE, ST_FILL);
  bool result = (s[r2][c2] == ST_FILL);
  lee(r1, c1, ST_FILL, ST_ALIVE);
  return result;
}
