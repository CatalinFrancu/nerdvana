// Dijkstra's algorithm. Store two values
#include <queue>
#include <stdio.h>
#include <vector>
...
struct node {
  std::vector<edge> adj;
  long long dist[2]; // minimum cost to get here without/with the discount
};
...
void dijkstra() {
  ...
  while (!pq.empty()) {
    ...
    // We don't use decrease-key, we simply insert nodes multiple times.
    // Hence, we must check if the distance still stands.
