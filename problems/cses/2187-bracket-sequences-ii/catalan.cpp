// Proof for regular Catalan numbers:
// https://en.wikipedia.org/wiki/Catalan_number#Second_proof
//
// In our problem, we can use regular Catalan numbers when the prefix is valid
// and balanced (including when possibly empty).
//
// When the prefix is imbalanced, let k be the number of excess open
// brackets. We can apply the same logic of flipping paths. We get the formula
//
// (k + 1) / (n + 1) * comb(2n - k, n)
#include <ctype.h>
#include <stdio.h>
...
void read_string() {
  ...
  // Reduce the problem to length n, starting with k open brackets.
  n = (n - k + excess) / 2;
  k = excess;
}
