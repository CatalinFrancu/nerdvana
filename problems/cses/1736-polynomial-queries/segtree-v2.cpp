...
struct segment_tree {
  ...
  void push(int t, int len) {
    ...
    v[t] += progression_sum(first[t], step[t], len);
    first[t] = 0;
    step[t] = 0;
  }

  void pull(int t, int len) {
    v[t] = v[2 * t] + progression_sum(first[2 * t], step[2 * t], len / 2)
      + v[2 * t + 1] + progression_sum(first[2 * t + 1], step[2 * t + 1], len / 2);
  }
  ...
  long long range_sum_helper(int t, int pl, int pr, int l, int r) {
    if (l >= r) {
      ...
      return v[t] + progression_sum(first[t], step[t], r - l);
    } else {
      push(t, pr - pl);
      int mid = (pl + pr) >> 1;
