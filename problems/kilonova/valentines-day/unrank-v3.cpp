// Complexity: O(n log n log* n). Comments at the end.
//
// v2: Improve factor_set to O(1) for insert / erase
// v3: Replace the std::set hand with a disjoint-set forest.
#include <set>
#include <stdio.h>
...
struct factor_set {
  int cnt[MAX_N + 1]; // cnt[x] = number of factors equal to x
  int pos[MAX_N + 1]; // position in lst[] of x
  int lst[MAX_N + 1]; // list of distinct factors
  int n;              // number of distinct factors (size of lst)
  int total;          // number of factors
...
struct disjoint_set_forest {
  ...
  // Always merges left to right. Be sure to pass v > u. No union by rank.
  void unite(int u, int v) {
    p[find(u)] = find(v);
  }
};

disjoint_set_forest hand; // values still in hand, to be placed in p
