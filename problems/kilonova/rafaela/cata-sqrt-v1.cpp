...
struct node {
  ...
  bool touched;         // true dacă există actualizări în subarbore
};
...
int n, num_ops, num_updates, total_pop;
FILE* fin;
...
// Notifică-l pe u că există actualizarea upd în fiul său v.
void process_child_update(int u, int v, update& upd) {
  for (query& q: nd[u].q) {
    if (upd.id < q.id) {
...
int merge_advance(int u, int v, int i) {
  while ((i < num_updates) && (nd[updates[i].u].tout <= nd[v].tout)) {
    process_child_update(u, v, updates[i++]);
  }
...
void answer_queries(int u, int max_untouched) {
  for (query& q: nd[u].q) {
    ...
    answer[q.id] = total_pop + q.outside_delta - nd[u].spop;
...
void commit_updates() {
  for (int i = 0; i < num_updates; i++) {
    nd[updates[i].u].pop += updates[i].delta;
    total_pop += updates[i].delta;
  }
}
