// Method: Put all the 4N directed lines together and sort them by slope.
// Remove duplicates: if multiple lines have the same slope, only keep the
// leftmost one.
//
// Then scan all the lines and build the intersection polygon. Maintain a
// stack of lines we collected so far and a stack of pairwise intersections
// between them, which will be vertices in the intersection polygon. If the
// newly added line has the last polygon vertex on the right-hand side, pop
// the vertex from the stack. After we are done popping, push the new line on
// the line stack and push its intersection with the last line to the vertex
// stack.
//
// When this is over, the resulting line may be 6-shaped. Trim both ends,
// removing vertices on one end which are on the right-hand side of the line
// at the other end.
//
// I assumed that the intersection always exists, which turned out to be
// correct.
#include <algorithm>
#include <deque>
#include <math.h>
...
struct line {
  long long a, b, c;
  ipoint p; // some point on the line
  double angle;
