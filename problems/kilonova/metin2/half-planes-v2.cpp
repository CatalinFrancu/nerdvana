// Same as v1, but uses a different approach to determine the leftmost line
// among two parallel lines. This removes the need to store an explicit point
// on every line.
#include <algorithm>
#include <deque>
#include <math.h>
...
struct line {
  ...
  // Given that ax+by+c = 0, we wish to know the sign of dx+ey+f. Multiply and
  // reduce the two functions:
  //
  // adx + bdy + cd = 0
  // adx + aey + af ? 0
  // ==>
  // (ae - bd)y + (af - cd) ? 0
  //
  // Since the lines are parallel, ae=bd always, so the question becomes:
  //
  // af - cd ? 0
  //
  // Take care that when we multiply by a, the sign may change. When a is 0 we
  // need to work with b instead, but we can avoid the corner case by working
  // with z = a + b.
  bool left_of(line other) {
    long long disc = (a + b) * other.c - (other.a + other.b) * c;
    return disc * (a + b) > 0;
  }
...
void sort_lines() {
  std::sort(l, l + num_lines, [](line& l1, line& l2) {
    return
      (l1.angle < l2.angle) ||
      ((l1.angle == l2.angle) && l1.left_of(l2));
  });
}
